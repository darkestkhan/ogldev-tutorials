...
package body Util is
  ...
  ---------------------------------------------------------------------------
  Speed: constant Float := 0.5;

  Key_On: array (Character) of Boolean := (others => False);
  ...
  begin
    ...
    if Key_On ('w') then
      Camera.Move (Game_Camera,  0.1 * Speed, 0.0);
    end if;
    ...
    Pipeline.Set_Camera
      ( Pipe,
        Camera.Get_Position (Game_Camera),
        Camera.Get_Target   (Game_Camera),
        Camera.Get_Up       (Game_Camera)
      );
  end Update;
  ...
  begin
    ...
    GL.Uniform (WVP_Location, 1, GL.GL_TRUE, Trans);
  ...
  is
    ...
    Dir           : constant String := "src/tut14/shaders/";
    Success       : Boolean := False;
  ...
  begin
    ...
    WVP_Location := GL.Get_Uniform_Location (Shader_Program, "WVP");
    if WVP_Location = -1 then
  ...
  begin
    ...
    Create_Vertex_Buffer: declare
      Vertices: constant array (Natural range <>) of Float :=
        ( -1.0, -1.0,  0.5773,
           0.0, -1.0, -1.15475,
           1.0, -1.0,  0.5773,
           0.0,  1.0,  0.0
